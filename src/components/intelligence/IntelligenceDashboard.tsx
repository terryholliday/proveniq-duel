...
import { Sparkles, Play, RotateCcw, ChevronRight, Code, Terminal, MessageSquare, Cpu, CheckCircle2, AlertCircle, ShieldCheck, Target, Volume2, VolumeX, History, BarChart3, GitCompare, Copy, Check } from "lucide-react";
...
export default function IntelligenceDashboard() {
    ...
    const [mode, setMode] = useState<"refine" | "orchestrate">("refine");
    ...
    const startRefinement = async () => {
        ...
        setBattleStatus("🎮 Both fighters enter the arena!");
        setBattleLog([]);
        setScores({ gemini: 0, openai: 0 });
        setCurrentRound(0);
        setCurrentAttacker("");
    ...
    return (
        <div className="flex flex-col gap-8 w-full max-w-6xl mx-auto p-6 min-h-screen bg-transparent text-zinc-100 font-sans selection:bg-zinc-800 selection:text-zinc-200">
            {/* Header */}
            <div className="flex flex-col gap-2 border-l-2 border-zinc-800 pl-6 py-2">
                ...
                <p className="text-zinc-400 text-sm tracking-wider uppercase">Iterative Refinement Registry v2.1</p>
            </div>
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    <div className="p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
                        ...
                        <label className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-3 block">
                            {mode === "refine" ? "Primary Objective" : "Business Goal"}
                        </label>
                        <textarea
                            ...
                            placeholder={mode === "refine" ? "Describe the logic or module to refine..." : "Describe the high-level business objective..."}
                            value={task}
                            onChange={(e) => setTask(e.target.value)}
                        />

                        <button
                            onClick={mode === "refine" ? startRefinement : startOrchestration}
                            disabled={loading || !task}
                            className={`mt-6 w-full py-3 rounded-full font-semibold text-sm flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] active:scale-[0.98] ${mode === "refine" ? "bg-zinc-100 text-black hover:bg-white" : "bg-red-500 text-white hover:bg-red-400 shadow-[0_0_20px_rgba(239,68,68,0.2)]"
                                }`}
                        >
                            {loading ? (
                                <RotateCcw className="w-4 h-4 animate-spin" />
                            ) : (
                                <Play className={`w-4 h-4 ${mode === "refine" ? "fill-current" : "fill-white"}`} />
                            )}
                            {loading ? "Synchronizing Engines..." : mode === "refine" ? "Initialize Refinement" : "Generate Admin Tasks"}
                        </button>
                    </div>

                    {/* Adjudication Truth Card */}
                    <AnimatePresence>
                        {adjudication && (
                            <motion.div
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                    {adjudication.meetsOriginalIntent ? (
                                        <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                    ) : (
                                        <AlertCircle className="w-4 h-4 text-rose-500" />
                                    )}
                                    <span className="text-[10px] font-bold uppercase tracking-wider">
                                        {adjudication.meetsOriginalIntent ? "Intent Fulfilled" : "Parity Misaligned"}
                                    </span>
                                </div>
                            </motion.div>
                        )}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {loading && mode === "refine" ? (
                            ...
                            <div className="h-full min-h-[600px] rounded-2xl border border-zinc-800 flex flex-col items-center justify-center text-center p-12 bg-zinc-950/20 backdrop-blur-3xl">
                                ...
                                <h2 className="text-xl font-light text-zinc-200 mb-2 tracking-tight">System Idle</h2>
